...
## รันในเครื่องตัวเอง (ลองของก่อน deploy)

**Windows — ดับเบิลคลิก `dev.cmd`** แล้วเปิด `http://localhost:5173`
สคริปต์จะบอกลิงก์สำหรับเปิดบนมือถือในวง wifi เดียวกันให้ด้วย

```bash
npm run dev -- --host    # macOS / Linux
```

แก้โค้ดแล้วหน้าจอเปลี่ยนทันที ไม่ต้อง build ไม่ต้องอัปขึ้นเว็บ
แต่ยังต้องต่อเน็ต เพราะข้อมูลสินค้ากับบิลอยู่ที่ Firebase
...
```
src/
├─ App.jsx                 ประตูล็อกอิน + สลับหน้า (lazy load ยกเว้นหน้าขาย)
...
│  ├─ sauceCost.js           สูตรคิดต้นทุนน้ำจิ้ม — ฟังก์ชันบริสุทธิ์ ไม่แตะฐานข้อมูล
│  ├─ sauces.js              อ่าน/เขียนสูตรและประวัติการทำน้ำจิ้ม
...
├─ pages/                  Sales · Reports · Documents · Inventory · Shift · Expenses · Sauce · Settings · Login
...
## ระบบน้ำจิ้ม — เปิด ปิด และถอนออก

ระบบนี้เพิ่มเข้ามาทีหลังเพื่อตอบคำถามว่า "ต้นทุนจริงต่อไม้เท่าไหร่" โดยแยกต้นทุน
ออกเป็นสามก้อน — วัตถุดิบหลัก, น้ำจิ้มที่ทำเอง, ของประกอบ ถูกออกแบบให้ถอยกลับได้จริง
ถ้าลองแล้วไม่เข้ากับวิธีทำงานของร้าน

**ถอยกลับได้ 3 ระดับ ไล่จากเบาไปหนัก**

1. **ปิดสวิตช์** ที่หน้าตั้งค่า → แท็บน้ำจิ้มหาย และ `unitCost` กลับไปให้ผลเท่าเดิมทุกบาท
   เพราะ `appData` จะส่ง `sauceById` เป็น `null` ข้อมูลที่บันทึกไว้ยังอยู่ครบ เปิดกลับได้ทุกเมื่อ
2. **ลบทีละชิ้น** ที่หน้าน้ำจิ้ม → ลบหม้อที่บันทึกผิด (ระบบเลื่อนไปใช้หม้อก่อนหน้าให้เอง)
   หรือลบทั้งสูตร (ตัดสูตรออกจากสินค้าที่ผูกไว้ให้ด้วย)
3. **ลบทั้งระบบ** ที่หน้าตั้งค่า → `purgeSauceSystem()` ลบ `sauces` + `sauce_batches`
   แล้วเคลียร์ `sauce_id` ในสินค้าทุกตัว ยอดขาย บิล และสต็อกไม่ถูกแตะ

**สต็อกวัตถุดิบ** — บันทึกซื้อแล้วสต็อกเพิ่ม บันทึกทำน้ำจิ้มแล้วสต็อกลด ทั้งสองทางใช้
`increment()` ไม่ใช่อ่านมาบวกแล้วเขียนกลับ เพราะเปิดสองเครื่องพร้อมกันแล้วของจะหายไปหนึ่งรอบ
การตัดสต็อกอยู่ในชุดเดียวกับที่บันทึกหม้อ เน็ตหลุดกลางคันจะไม่เหลือหม้อที่ไม่ได้ตัดของ
ของขวดที่ใช้ได้หลายหม้อจะตัดแค่เศษส่วน (น้ำปลา 1 ขวดใช้ 10 หม้อ → หม้อละ 0.1 ขวด)
สต็อกไม่พอไม่ห้ามบันทึก เพราะกว่าจะมากรอกคือกวนเสร็จไปแล้ว แต่ขึ้นเตือนให้ไปนับใหม่

**ถ้าจะถอนโค้ดออกด้วย** ลบไฟล์ `lib/sauces.js` `lib/sauceCost.js` `pages/SaucePage.jsx`
`components/SauceBatchModal.jsx` แล้วเอา `sauceById` ออกจาก `pricing.js` กับที่เรียกใช้
(`ReportsPage` `InventoryPage` `BundleModal` `EditProductModal`) และเอาแท็บ `sauce`
ออกจาก `App.jsx` `BottomNav.jsx` `Sidebar.jsx` — ทุกจุดอยู่หลังเงื่อนไข `sauceEnabled`
อยู่แล้ว จึงไม่มีที่ไหนที่ต้องรื้อตรรกะเดิม
